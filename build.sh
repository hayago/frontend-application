...
# Use staging
COGNITO_USER_POOL_ID=`aws ssm get-parameter --name stagingssmCognitoUserPoolId --query "Parameter.Value" --output text`
export COGNITO_USER_POOL_ID=${COGNITO_USER_POOL_ID}
COGNITO_APP_CLIENT_ID=`aws ssm get-parameter --name stagingssmCognitoUserPoolAppId --query "Parameter.Value" --output text`
export COGNITO_APP_CLIENT_ID=${COGNITO_APP_CLIENT_ID}
COGNITO_IDENTITY_POOL_ID=`aws ssm get-parameter --name stagingssmCognitoIdentityPoolId --query "Parameter.Value" --output text`
export COGNITO_IDENTITY_POOL_ID=${COGNITO_IDENTITY_POOL_ID}
ALL_TOKEN_HISTORY_CSV_DWONLOAD_S3_BUCKET=`aws ssm get-parameter --name stagingssmAllTokenHistoryCsvDownloadS3Bucket --query "Parameter.Value" --output text`
export ALL_TOKEN_HISTORY_CSV_DWONLOAD_S3_BUCKET=${ALL_TOKEN_HISTORY_CSV_DWONLOAD_S3_BUCKET}
export WEB3_SERVICE_BASE_URL=`aws ssm get-parameter --name stagingssmWeb3ServiceBaseUrl --query "Parameter.Value" --output text`
BURN_ADDRESS=`aws ssm get-parameter --name stagingssmBurnAddress --query "Parameter.Value" --output text`
export BURN_ADDRESS=${BURN_ADDRESS}
SENTRY_DSN=`aws ssm get-parameter --name stagingssmSentryDSN --query "Parameter.Value" --output text`
